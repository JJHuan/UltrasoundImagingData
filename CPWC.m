%close all;
function [b_data, channel_data, Na] = CPWC (ra, zca, xca)
    %Basic Constants
    ...
    %Transducer definition custom 64-element linear array transducer
    %Our next step is to define the ultrasound transducer array we are using. For this experiment, we shall use 64-element 4 MHz 0.25 mm pitch size  custom Transducer and set our parameters to match it.
    probe = uff.linear_array();
    f0                      = 4e+06;      % Transducer center frequency [Hz]
    lambda                  = c0/f0;           % Wavelength [m]
    probe.element_height    = 5e-4;            % Height of element [m]
    probe.pitch             = 0.250e-3;        % probe.pitch [m]
    kerf                    = 0.1e-03;        % gap between elements [m]
    ...
    %{
    figure;
    plot((0:(length(two_way_ir)-1))*dt -lag*dt,two_way_ir); hold on; grid
    on; axis tight %plot 2-way Ultrasound pulse
...
%}
    ...
    F_number = 1.7;                             %F_number=focal depth/(array aperture width /2). array aperture width=total active elemnt width. For CPWC imaging the active elemnt width is the total array length.
    alpha_max = atan(1/2/F_number);
    A = [8 16];                                 % Array of the number of plane waves.
    Na=randsample(A,1);                         % Randomly select the number of angles from the array.
    ...
    %{
    point_position(1,:) = [0 0 10e-3]; %define point target phantom
    ...
    %}
    [point_position, point_amplitudes] = cyst_pht(10000, ra, zca, xca); % call the cyst_pht function by reference.
    ...
    sca=uff.linear_scan('x_axis',linspace(-20e-3,20e-3,256).', 'z_axis', linspace(30e-3,90e-3,256).');
